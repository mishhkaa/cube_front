...
export const CsdProjects = () => {
  const navigate = useNavigate();
  const [data, setData] = useState({ data: [], total: 0 });
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(1);
  const type = 'csd-projects';

  useEffect(() => {
    setLoading(true);
    MedianApi.integrationsCsdProjects({ page, per_page: 20 })
      .then(res => setData({
